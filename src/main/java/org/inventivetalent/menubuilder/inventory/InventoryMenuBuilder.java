...
public class InventoryMenuBuilder extends MenuBuilder<Inventory> {
	...
	public InventoryMenuBuilder withEventHandler(InventoryEventHandler eventHandler) {
		try {
			DungeonAPI.getInstance().inventoryListener.registerEventHandler(this, eventHandler);
	...
	public InventoryMenuBuilder onInteract(InventoryMenuListener listener, ClickType... actions) {
		...
		try {
			DungeonAPI.getInstance().inventoryListener.registerListener(this, listener, actions);
		} catch (IllegalArgumentException e) {
			throw e;
		}
		return this;
	...
	@Override
	public void dispose() {
		DungeonAPI.getInstance().inventoryListener.unregisterAllListeners(getInventory());
	}

	public void unregisterListener(InventoryMenuListener listener) {
		try {
			DungeonAPI.getInstance().inventoryListener.registerListener(this, listener, ALL_CLICK_TYPES);
		} catch (IllegalArgumentException e) {
			throw e;
		}
	}
